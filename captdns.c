...
/* =====[ AKOSTAR WiFi Clock project ]====================================
   ...
   Revisions:
      ...
      ----  -----------     ----------      ------------------------------
      0.00  mar.7.2018     Peter Glen      Initial version.
...
char buff[DNS_LEN + 1];
char reply[DNS_LEN + 1];
	
//Receive a DNS packet and maybe send a response back
static int  captdnsRecv(struct sockaddr_in *premote_addr, char *pusrdata, unsigned short length) 
{
    int ret = 0;
	int i;
	...
	p+=sizeof(DnsHeader);
    //	ESP_LOGI(myTAG, "DNS packet: id 0x%X flags 0x%X rcode 0x%X qcnt %d ancnt %d nscount %d arcount %d len %d, 
	...
	//Some sanity checks:
	if (length>DNS_LEN) return ret; 								//Packet is longer than DNS implementation allows
	if (length<sizeof(DnsHeader)) return ret; 						//Packet is too short
	if (hdr->ancount || hdr->nscount || hdr->arcount) return ret;	//this is a reply, don't know what to do with it
	if (hdr->flags&FLAG_TC) return ret;								//truncated, can't use this
	...
	for (i=0; i<my_ntohs(&hdr->qdcount); i++) 
	{
		...
		//ESP_LOGI(myTAG, "DNS: Q (type 0x%X class 0x%X) for %s\n", my_ntohs(&qf->type), my_ntohs(&qf->class), buff);
		//ESP_LOGI(myTAG, "DNS: Q for '%s'\n", buff);
		...
		if (my_ntohs(&qf->type)==QTYPE_A) {
			...
			*rend++=ip4_addr4(&info.ip);
			...
			//ESP_LOGI(myTAG, "IP Address:  %s, ip4addr_ntoa(&info.ip));
            //ESP_LOGI(myTAG, "Added A rec to resp. Resp len is %d, (rend-reply)");
			...
			*rend++=0;
			setn16(&rhdr->ancount, my_ntohs(&rhdr->ancount)+1);
            //ESP_LOGI(myTAG, "Added NS rec to resp. Resp len is %d, (rend-reply)");
			...
			//rend+=16;
			memcpy(rend, "192.168.4.1", 11);
			rend+=11;
			setn16(&rhdr->ancount, my_ntohs(&rhdr->ancount)+1);
            //ESP_LOGI(myTAG, "Added NS rec to resp. Resp len is %d\n", (rend-reply));
		}
	}	
	//Send the response
	//ESP_LOGI(myTAG, "Send response\n");
	ret = sendto(sockFd,(uint8_t*)reply, rend-reply, 0, (struct sockaddr *)premote_addr, sizeof(struct sockaddr_in));
    return ret;
}
...
static void captdnsTask(void *pvParameters) {
    ...
    ESP_LOGI(myTAG, "CaptDNS init started.");
	...
	do {
        ...
        if(sockFd != 0)
            {
            ESP_LOGI(myTAG, "CaptDNS reclosed socket.");
	        close(sockFd);
            }
		sockFd = socket(AF_INET, SOCK_DGRAM, 0);
		if (sockFd==-1) {
			ESP_LOGI(myTAG, "captdns_task failed to create sock!");
	...
	do {
		...
		if (ret!=0) {
			ESP_LOGI(myTAG, "captdns_task failed to bind sock!");
			vTaskDelay(1000/portTICK_RATE_MS);
		}
        if(cnt++ > 10)
            goto endd;
    ...
    ESP_LOGI(myTAG, "CaptDNS inited, listening ...");
	...
	while(1) 
		...
		ret=recvfrom(sockFd, (uint8_t *)udp_msg, DNS_LEN, 0, 
                    (struct sockaddr *)&from,(socklen_t *)&fromlen);
		if (ret>0) 
            {
            int ret2 = captdnsRecv(&from, udp_msg,ret);
            if(ret2 <= 0)
                {
                ESP_LOGI(myTAG, "CaptDNS failed on recv.");
                vTaskDelay(100/portTICK_RATE_MS);
                }
         }
        else
            {
            ESP_LOGI(myTAG, "CaptDNS failed on recvfrom.");
	        vTaskDelay(100/portTICK_RATE_MS);
            //break;
            }
	}
  ...
  endd:
	ESP_LOGI(myTAG, "CaptDNS loop ended.");
	close(sockFd);
    sockFd = 0;
	vTaskDelete(NULL);
}
...
{
    termdns = false;
    //ESP_LOGI(myTAG, "CaptDNS task creation.);
    xTaskCreate(captdnsTask, (const char *)"captdns_task", 
                                    10000, NULL, 3, &xHandle2);
    vTaskDelay(100/portTICK_RATE_MS);
}
